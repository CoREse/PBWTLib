...
Array arrayCopy (Array a) 
{
  ...
  newArray = uArrayCreate (a->dim, a->size) ;
  memcpy (newArray->base, a->base, a->dim * a->size) ;
  newArray->max = a->max ;
  return newArray;
}
...
void arrayExtend (Array a, long n) 
{
  ...
  newArray = (char*) _mycalloc (a->dim, a->size) ;
  memcpy (newArray,a->base,a->size*a->max) ;
  free (a->base) ;
  a->base = newArray ;
