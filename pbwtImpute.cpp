...
/******* phase() is phaseOld() rewritten with CNew API and method 3 only ******/
...
PBWT *phase (PBWT *p, int nSparse) /* return rephased p */
{
  ...
  /* compare CNew phasing to original and report switch rates */
...
/******* phase a CNew pbwt against the existing one as a reference *******/
...
static ExtendStruct *extendUpdate (int dd, int x)
{
  ...
  if (dd > ex->d)	/* make a CNew exStack element */
...
/* so replace with CNew phaseExtend() below, I hope */
{
  ...
  if (!CNew->s)		/* this is the first j to map here */
    { CNew->s = sBit ; CNew->sBest = sBit ;
      CNew->j1 = j1New ;
      CNew->back = (x0 == x1) ? old->back : traceBackCreate (old->back, x0, j0, k) ;
      return 1 ;
    }
  /* XXX for sampling calculate chance to pick this j rather than
     previous here */
  if (sBit > CNew->sBest) /* this is better than previous j that mapped here */
    { CNew->s = sBit ; 	 /* YYY originally +=, for Viterbi change to = */
      CNew->sBest = sBit ;
      CNew->j1 = j1New ;
      if (traceBackCell(CNew->back)->back == old->back) /* be careful - edge case! */
	{ traceBackCell(CNew->back)->value = x0 ; return 0 ; }
      else
	{ traceBackPrune (CNew->back, k, j0) ;
	  CNew->back = (x0 == x1) ? old->back : traceBackCreate (old->back, x0, j0, k) ;
	  return 1 ;
	}
    }
  /* CNew->s += sBit ;	   /* include if not Viterbi */
  return 0 ;
}
#endif // EXTEND0
...
/* returns the number of CNew back pointers created */
{
  ...
  if (!CNew->s)		/* this is the first j0 to map here */
    { *CNew = temp ;
      CNew->back = (x0 == x1) ? old->back : traceBackCreate (old->back, x0, j0, k) ;
      return 1 ;
    }
  else if (temp.s > CNew->s)	/* this is better than previous j that mapped here */
    { TraceBackIndex oldNewBack = CNew->back ;
      *CNew = temp ; 	/* originally +=, for Viterbi change to = */
      if (traceBackCell(oldNewBack)->back == old->back) /* be careful - edge case! */
	{ CNew->back = oldNewBack ; traceBackCell(CNew->back)->value = x0 ; return 0 ; }
      else
  ...
  PBWT *referenceImpute (PBWT *pOld, char *fileNameRoot, int nSparse, double fSparse)
{
  ...
  if (!pRef->sites) die ("CNew pbwt %s in referencePhase has no sites", fileNameRoot) ;
  if (strcmp(pOld->chrom,pRef->chrom))
    die ("mismatching chrom in referenceImpute: old %s, CNew %s", pRef->chrom, pOld->chrom) ;
...
void genotypeCompare (PBWT *p, char *fileNameRoot)
{
  ...
  if (!pRef->sites) die ("CNew pbwt %s in genotypeCompare has no sites", fileNameRoot) ;
